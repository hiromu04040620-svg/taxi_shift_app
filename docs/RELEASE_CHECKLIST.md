...
- [ ] `fix/app-review-iap-support` の差分をレビュー
...
- [ ] おおよその位置情報、端末ID、広告データ、製品操作、クラッシュ、パフォーマンス、その他の診断情報をAdMob由来として申告
- [ ] 端末IDをトラッキング目的として申告
- [ ] ATT の表示文言が実態と合っていることを確認
- [ ] AdMobの「プライバシーとメッセージ」で対象地域向けメッセージを公開
...
- [ ] Debug、TestFlight、Sandbox、App ReviewではGoogle公式テスト広告IDになることを確認
- [ ] App Store本番レシートでは本番広告IDになることを確認
- [ ] ATT 許可前後の広告表示を確認
- [ ] UMP同意前は広告を要求せず、同意変更入口が必要な地域だけ設定に表示されることを確認
- [ ] 課金なしでも主要機能が使えることを確認
...
- [ ] ホーム画面から主要機能、Sandbox購入成功、広告消失までを実機録画して審査情報へ添付
